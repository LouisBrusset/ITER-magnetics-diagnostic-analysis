...
class TimeSeriesDataset(Dataset):
    def __init__(self, data: np.ndarray, start_idx: int = 0, end_idx: int = -1):
        ...
        ).float()  # Convert to float32 to be compatible with PyTorch model parameters
...
def create_data_loaders(
    ...
    if len(train_dataset) % batch_size != 0:
        print(
            f"Warning: Batch size {batch_size} does not divide train dataset size {len(train_dataset)}"
        )
    if len(valid_dataset) % batch_size != 0:
        print(
            f"Warning: Batch size {batch_size} does not divide validation dataset size {len(valid_dataset)}"
        )
    if len(test_dataset) % batch_size != 0:
        print(
            f"Warning: Batch size {batch_size} does not divide test dataset size {len(test_dataset)}"
        )
...
if __name__ == "__main__":
    ...
    train_loader, valid_loader, test_loader = create_data_loaders(
        data=data, batch_size=10, set_separations=[12000, 15000], gap_time=10
    )
    ...
    print(
        f"Validation batches: {len(valid_loader)}, Total samples: {len(valid_loader.dataset)}"
    )
    print(
