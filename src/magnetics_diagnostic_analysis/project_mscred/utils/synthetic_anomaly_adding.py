...
def create_anomalies(
    ...
    for i in range(n_anomalies):
        ...
        anomaly_type = np.random.choice(
            ["spike", "drift", "level_shift", "seasonal_break"]
        )
        ...
        if anomaly_type == "spike":
            ...
            drift_slope = (
                anomaly_strength * local_std * 0.01 * np.random.choice([-1, 1])
            )
            ...
            seasonal_anomaly = seasonal_amp * np.sin(
                2 * np.pi * freq_change * time_segment / 100
            )
            data_with_anomalies[
                series_num, start_anomaly:end_anomaly
            ] += seasonal_anomaly
...
def plot_anomalies(data_normal, data_anomalous, anomalies_info, n_series_to_plot=3):
    ...
    for i, series_num in enumerate(series_to_plot):
        ...
        axes[i].plot(
            data_anomalous[series_num], "r-", alpha=0.9, label="With Anomalies"
        )
