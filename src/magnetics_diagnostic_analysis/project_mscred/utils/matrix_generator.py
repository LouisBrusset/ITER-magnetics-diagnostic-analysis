    ...
    for w_idx, win in enumerate(win_size):
        ...
        if saving:
            output_path = (
                ...
                / "data/preprocessed/mscred/windows_matrix"
            )
...
def create_signature_matrix_animation(
    ...
    def update(frame):
        ...
        ax.set_title(
            f"Signature Matrix (Window size: {win_size})\nTime: {frame * gap_time}"
        )
        return [im]
...
if __name__ == "__main__":
    ...
    path = (
        Path(__file__).absolute().parent.parent.parent.parent.parent
        / "data/preprocessed/mscred/signature_matrices.npy"
    )
