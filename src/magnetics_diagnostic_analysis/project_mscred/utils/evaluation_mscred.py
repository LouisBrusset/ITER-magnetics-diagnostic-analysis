...
from magnetics_diagnostic_analysis.project_mscred.utils.dataloader_building import (
    create_data_loaders,
)
...
def detect_anomalies_all(
    ...
    with torch.no_grad():
        anomalies_full = np.zeros(len(data_loader) * config.BATCH_SIZE)
        reconstructed_full = np.zeros(
            (len(data_loader) * config.BATCH_SIZE, *config.DATA_SHAPE)
        )
        i = 0
        for x in data_loader:
            x = x.to(device)
            batch_size = x.size(0)
            x_recon = model(x)
            reconstructed_full[i * batch_size : (i + 1) * batch_size] = (
                x_recon.cpu().numpy()
            )
...
def plot_anomalies_all(
    ...
    plt.axhline(
        y=threshold, color="k", linestyle="--", label="Threshold (%.2f)" % threshold
    )
    legend_added = False
    ...
    path = (
        ...
        / "results/figures/mscred/anomaly_scores.png"
    )
    plt.savefig(path)
    return None


def compute_residuals(reconstructed: np.ndarray, original: np.ndarray) -> np.ndarray:
    """
...
def detect_problematic_diagnostics(
    residuals: np.ndarray, anomaly_mask: np.ndarray, threshold_percentile: float = 90.0
    ...
    for sensor_i in range(num_sensors):
        ...
        residuals_involving_i = np.concatenate(
            [total_residuals_matrix[sensor_i, :], total_residuals_matrix[:, sensor_i]]
        )
...
if __name__ == "__main__":
    torch.backends.cudnn.enabled = (
        False  # problem to fix in the future: due to LSTM cells?
    )
    ...
    reconstructed, anomalies, ano_mask = detect_anomalies_all(
        mscred, test_loader, threshold=ano_threshold, device=device
    )
    ...
    residuals = compute_residuals(
        reconstructed, data[config.SET_SEPARATIONS[1] // config.BATCH_SIZE :]
    )
    print("\nResiduals computed.")

    problematic_sensors, sensor_scores = detect_problematic_diagnostics(
        residuals, ano_mask, threshold_percentile=90
    )
