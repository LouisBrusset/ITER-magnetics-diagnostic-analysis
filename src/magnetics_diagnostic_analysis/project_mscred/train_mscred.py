...
def plot_history(history_train: list, history_valid: list) -> None:
    ...
    path = (
        ...
        / "results/figures/mscred/last_training_history.png"
    )
    plt.savefig(path)
    return None
...
def main():
    ...
    if continue_training:
        mscred.load_state_dict(
            torch.load(config.DIR_MODEL_PARAMS / f"{model_name_to_continue}.pth")
        )
    mscred.load_state_dict(
        torch.load(
            Path(__file__).absolute().parent / "checkpoints/model_checkpointed.pth"
        )
    )
    ...
    try:
        ...
        torch.save(
            trained_mscred.state_dict(),
            config.DIR_MODEL_PARAMS / f"{model_name_register}.pth",
        )
        ...
        if "trained_mscred" in locals():
            torch.save(
                trained_mscred.state_dict(),
                config.DIR_MODEL_PARAMS / f"{model_name_register}_interrupted.pth",
            )
        else:
            torch.save(
                mscred.state_dict(),
        ...
        print(
            f"Model saved to: {config.DIR_MODEL_PARAMS / f'{model_name_register}_interrupted.pth'}"
        )
        return
