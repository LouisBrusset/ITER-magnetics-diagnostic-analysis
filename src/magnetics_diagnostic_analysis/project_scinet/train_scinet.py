...
from magnetics_diagnostic_analysis.project_scinet.utils.build_dataset import (
    PendulumDataset,
)
...
def train_scinet(
    ...
    print(
        f"{'Epoch':<20} ||| {'Train Loss':<15} ||| {'KLD Loss':<12} {'Recon Loss':<12} ||||||| {'Valid Loss':<15}"
    )
    ...
    for epoch in range(num_epochs):
        ...
        for observations, questions, a_corr, _ in tqdm(
            ...
            loss, l_kld, l_recon = scinet_loss(
                possible_answer, a_corr, mean, logvar, beta=kld_beta
            )
            loss.backward()
        ...
        print(
            f"{f'{epoch+1}/{num_epochs}':<20}  |  {train_loss:<15.6f}  |  {kld_loss:<12.6f} {recon_loss:<12.6f}    |    {valid_loss:<15.6f}"
        )
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.Adam(
        pendulum_net.parameters(),
        lr=config.FIRST_LEARNING_RATE,
        weight_decay=config.WEIGHT_DECAY,
    )
    early_stopper = EarlyStopping(
        patience=config.ES_PATIENCE, min_delta=config.ES_MIN_DELTA
    )
    gradient_clipper = GradientClipping(max_norm=config.GC_MAX_NORM)
    lr_scheduler = LRScheduling(
        optimizer,
        factor=config.LRS_FACTOR,
        patience=config.LRS_PATIENCE,
        min_lr=config.LRS_MIN_LR,
        min_delta=config.LRS_MIN_DELTA,
    )
