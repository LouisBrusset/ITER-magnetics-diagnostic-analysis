...
if __name__ == "__main__":
    ...
    possible_answer = make_one_prediction(
        pendulum_net, observations[0], questions[0], device=device
    )
    plot_one_prediction(
        observations[0],
        questions[0],
    ...
    N_samples = 1
    observations, _, _, params = build_dataset(
        N_samples, kapa_range, b_range, maxtime=maxtime, timesteps=timesteps
    )
    answers = data_synthetic_pendulum(
        params[0][0], params[0][1], t=np.linspace(maxtime, maxtime * 2, timesteps)
    )

    questions = np.linspace(0, maxtime * 2, timesteps)
    possible_answers = make_timeserie_prediction(
        pendulum_net, observations[0], questions, device=device
    )
    plot_timeserie_prediction(
        observations[0], answers, possible_answers, maxtime=maxtime, timesteps=timesteps
    )
